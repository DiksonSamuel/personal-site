...
const Projects = ({ data }) => {
  ...
  const renderSingleProject = (project, index) => {
    ...
    let imageSize = project.potrait == 1 ? 'h-96' : 'h-96';
      ...
      <div  className='w-3/5'>
        <div className={'flex row justify-center items-center my-10 ' + reverse}>
          <div className='row flex border-b-2 border-black px-10' key={index} >
            <div className='border-r-2 border-black py-10'>
          ...
          <img src={project.image} alt={project.name} className={'mx-auto ' + imagemargin + ' ' + imageSize} />
        </div>
